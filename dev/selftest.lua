...
function selftest.run()
  ...
  for _, quality in ipairs(qualities.all()) do
    if prototypes.quality[quality.name].hidden then
      offers_hidden, hidden_name = true, quality.name
    end
  end
  check("hidden qualities are never offered", not offers_hidden, hidden_name)
  ...
  local function ghost_at(x, y)
    return surface.find_entity("entity-ghost", { x + 0.5, y + 0.5 })
  end
  ...
  if other and normal_result then
    ...
    if pair then
      ...
      if split_result then
        ...
        local splitter_ghost = surface.find_entity("entity-ghost", { BX + 9.5, BY + 21 })
