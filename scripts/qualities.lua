...
local function build()
  ...
  for name, proto in pairs(prototypes.quality) do
    -- "normal" is kept whether or not it is hidden: the base game ships it
    -- hidden (the quality mod unhides it), and it is the quality every ghost is
    -- placed at when nothing else is chosen, so a registry without it would have
    -- no default. The count of selectable qualities still comes out as one on a
    -- base-only install, which is what keeps the row out of the window there.
    if name == NORMAL or not proto.hidden then
